...
const EmployeeEdit: React.FC = () => {
  ...
  const fetchRoles = async () => {
    ...
    try {
      const roles = await roleApiService.getRoles(currentTenant.id, currentStore.store_id);
  ...
  }, [id, isEdit, currentTenant?.id, currentStore?.store_id]);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <DropdownSearch
                ...
                options={DEPARTMENTS.map(dept => ({
                  id: dept,
                  label: dept,
                  description: `Assign employee to ${dept} department`
                }))}
                value={formData.department}
                onSelect={(option) => {
                  if (option) {
                    handleInputChange('department', option.id);
                  } else {
                    handleInputChange('department', 'Sales'); // Default fallback
                  }
                }}
              ...
              <DropdownSearch
                ...
                onSelect={(option) => {
                  if (option) {
                    handleInputChange('role', option.id);
                    handleInputChange('role_name', option.label);
                  } else {
                    handleInputChange('role', '');
                    handleInputChange('role_name', '');
                  }
                }}
                error={errors.role}
                required
