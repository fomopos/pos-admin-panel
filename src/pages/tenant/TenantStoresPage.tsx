 ...
 * TenantStoresPage
 * 
 * Standalone page for managing stores under the current tenant.
 * Store changes affect billing (seat-based pricing).
 */

import React from 'react';
import { useNavigate } from 'react-router-dom';
import { PageHeader, Loading } from '../../components/ui';
import { StoreList } from '../../components/tenant-dashboard';
import { useTenantRole } from '../../hooks/useTenantRole';
import { useTenantStore } from '../../tenants/tenantStore';
import { ShieldExclamationIcon, BuildingOfficeIcon } from '@heroicons/react/24/outline';

const TenantStoresPage: React.FC = () => {
  const navigate = useNavigate();
  const { currentTenant } = useTenantStore();
  const { hasDashboardAccess, isLoading: roleLoading, role } = useTenantRole();

  if (roleLoading) {
    return <Loading title="Loading Store Management" description="Determining your access level..." variant="primary" />;
  }
      ...
      <div className="space-y-6 p-4 sm:p-6 bg-gray-50 min-h-screen">
        <div className="max-w-md mx-auto text-center py-24">
          ...
          <p className="text-gray-500 mb-6">
            Store management is available to <strong>Owner</strong> and <strong>Admin</strong> roles only.
    ...
    return (
      <div className="space-y-6 p-4 sm:p-6 bg-gray-50 min-h-screen">
        <div className="max-w-md mx-auto text-center py-24">
          ...
          <p className="text-gray-500 mb-6">Please select an organization to manage stores.</p>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        ...
        description={`Manage stores for ${currentTenant.name} — changes may affect your billing`}
      />
      <StoreList />
    </div>
  );
