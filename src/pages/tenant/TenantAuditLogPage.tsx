 ...
 * TenantAuditLogPage
 * 
 * Standalone page for viewing the tenant-level audit/activity log.
 * Read-only — shows actions like user invites, role changes, store modifications, etc.
 */

import React from 'react';
import { useNavigate } from 'react-router-dom';
import { PageHeader, Loading } from '../../components/ui';
import { AuditLog } from '../../components/tenant-dashboard';
...
const TenantAuditLogPage: React.FC = () => {
  ...
  if (roleLoading) {
    return <Loading title="Loading Activity Log" description="Determining your access level..." variant="primary" />;
  }

  if (!hasDashboardAccess || !can('canViewAuditLog')) {
    return (
      <div className="space-y-6 p-4 sm:p-6 bg-gray-50 min-h-screen">
        <div className="max-w-md mx-auto text-center py-24">
          ...
          <p className="text-gray-500 mb-6">
            Activity log is available to <strong>Owner</strong> and <strong>Admin</strong> roles only.
    ...
    return (
      <div className="space-y-6 p-4 sm:p-6 bg-gray-50 min-h-screen">
        <div className="max-w-md mx-auto text-center py-24">
          ...
          <p className="text-gray-500 mb-6">Please select an organization to view activity logs.</p>
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Activity Log"
        description={`Audit trail for all actions across ${currentTenant.name}`}
      />
      <AuditLog />
    </div>
  );
